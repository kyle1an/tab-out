  ...
  return (
    <div className="flex min-h-screen items-center justify-center p-8">
      <div className="w-full max-w-md rounded-2xl border border-(--warm-gray) bg-tab-card p-6 [corner-shape:squircle]">
        <h1 className="text-base font-semibold text-tab-ink">Tab Out hit an error</h1>
        <p className="mt-2 text-sm leading-5 text-tab-muted">
          The dashboard crashed while rendering. Reloading usually recovers it.
        </p>
        {message && (
          <pre className="mt-3 overflow-x-auto rounded-lg bg-[rgba(115,115,115,0.08)] p-2 text-xs leading-4 whitespace-pre-wrap text-tab-muted">{message}</pre>
        )}
        <button
          type="button"
          className="mt-4 inline-flex h-8 cursor-pointer items-center justify-center rounded-xl border border-(--warm-gray) bg-transparent px-3 text-sm font-medium text-tab-ink [corner-shape:squircle] hover:border-(--accent-amber) hover:bg-[rgba(82,82,82,0.08)] focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-(--accent-amber)"
