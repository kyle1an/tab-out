...
- The optional macOS Native Placement Bridge source lives under `native/bridge-host/`. `pnpm native-host:build` creates an ignored local executable, and `pnpm native-host:test` verifies its stdio-to-Unix-socket round trip; it is deliberately outside the cross-platform extension build.
...
- Native Placement Bridge changes additionally require `pnpm native-host:test` on macOS and the Hammerspoon bridge/router regression tests.
