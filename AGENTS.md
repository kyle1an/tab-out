...
- Activation History survives service-worker and extension reloads within one browser session, but resets on browser startup because Chrome tab and window IDs are session-scoped and may otherwise be reused for unrelated pages. Persisted history pairs each tab ID with its effective URL, clears the former ID-only schema once, and drops ID/URL mismatches before snapshot or shortcut navigation so re-enabling Tab Out after a missed startup event cannot target an unrelated reused tab ID.
