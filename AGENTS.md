...
- Duplicate handling renders one chip per effective URL with a count badge and keeps one copy according to the shared dedupe policy. The `New tabs` card may split same-URL Tab Out pages into current, Chrome-pinned, Chrome-grouped, and ordinary buckets so preserved copies stay visible. Bulk close and dedupe preserve Chrome tab groups; Tab Out dedupe preserves pinned Tab Out tabs and never closes the current active Tab Out page.
- Same-title pages with different effective URLs inside one rendered group merge visually into one title row with per-URL distinguishers. They are not URL duplicates; focus, close/delete, and page-pin actions stay scoped to the selected distinguisher, and pinned distinguishers split back into the local sibling list for ordering.
