...
- Page Chip and Activation History row context menus offer Copy title, Save page (where eligible), and Suspend. Suspend targets the live, not-already-suspended tabs behind the chip or row (folded chips suspend every matching live tab) and redirects them through the detected suspender target; without a detected suspender it only shows a toast.
- Suspended tabs keep their real page favicon in chips, history rows, and Working Set rows: rendering resolves the icon from the unwrapped URL through Chrome's favicon cache instead of keeping the suspender page's faded `data:` copy.
- Same-title pages with different effective URLs inside one rendered group merge visually into one title row with per-URL distinguishers. They are not URL duplicates; focus, close/delete, and page-pin actions stay scoped to the selected distinguisher, the merged row's favicon close action closes or deletes every closable variant at once, clicking the merged title or blank non-URL surface activates the default distinguisher (active current-window variant, active other-window variant, then first visible variant), and pinned distinguishers split back into the local sibling list for ordering.
