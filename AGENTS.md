...
- On initial Tabs-source load, Tab Out reserves the Activation History column from first paint and commits dashboard data, activation history, Working Set, and recently closed rows through one startup snapshot so the filter/header do not shift and history-only intermediate states do not flash; first paint reuses any structurally valid cached startup snapshot — the in-session `chrome.storage.session` copy, or a durable `chrome.storage.local` mirror (within a multi-day cap) when a browser restart cleared the session copy — rather than dropping it on a display-side freshness TTL, so even the first open after a restart paints warm with the last session's config-grouped snapshot instead of flashing an empty dashboard before live data arrives; live startup hydration runs after initial order memory is installed, keeps first-paint cached Working Set priority and holds the tabs chip order (ignoring remembered chip-order memory) for ordering until a filter or source change lifts the freeze, and does not animate card moves. The service worker also maintains this snapshot in the background, on browser startup and debounced tab/window events, so even the first open of a session can paint warm before any page has run; it reuses the same shared builder as the page, and defers to the page-written copy when function-based local path groupers are active (which it cannot run), so it never writes a snapshot that would group differently and shift on hydration.
...
- Suspended tabs keep the same favicon Chrome exposes for that tab in chips, history rows, and Working Set rows. Rendering uses Chrome's `tab.favIconUrl` when present and only falls back to the unwrapped URL through Chrome's favicon cache when Chrome has no favicon URL yet.
...
- Plain click focuses a chip's or history row's existing tab. Cmd-click (macOS) / Ctrl-click moves that tab into the current window in the background, Shift-click moves it into the current window and switches to it, and when no live tab exists the same gesture opens the URL in a new tab instead. The move gesture suppresses native text selection on the chip/row surface.
