...
- Plain click focuses a chip's or history row's existing tab. Cmd-click (macOS) / Ctrl-click moves that tab into the current window in the background and unsuspends it when needed, Shift-click moves it into the current window and switches to it, and when no live tab exists the same gesture opens the URL in a new tab instead. The move gesture suppresses native text selection on the chip/row surface.
