...
- On initial Tabs-source load, Tab Out reserves the Activation History column from first paint and commits dashboard data, activation history, Working Set, and recently closed rows through one startup snapshot so the filter/header do not shift and history-only intermediate states do not flash; first paint reuses any structurally valid cached session startup snapshot (`chrome.storage.session` clears on browser restart) rather than dropping it on a display-side freshness TTL, so reopens do not flash an empty dashboard before live data arrives; live startup hydration runs after initial order memory is installed, keeps first-paint cached Working Set priority and holds the tabs chip order (ignoring remembered chip-order memory) for ordering until a filter or source change lifts the freeze, and does not animate card moves.
