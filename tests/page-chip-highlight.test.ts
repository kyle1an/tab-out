...
test('TabHistoryPanel uses PageChip-style fade truncation and in-place title expansion', () => {
  ...
  assert.match(tabHistoryPanelSource, /HISTORY_ENTRY_INTERACTION_CLASSES = 'hover:bg-\(--history-entry-interaction-bg\)[\s\S]*\[\&\.history-entry-expanded-open\]:bg-\(--history-entry-interaction-bg\)/)
  ...
  assert.match(tabHistoryPanelSource, /history-entry-list pointer-events-none relative flex min-h-0[^"]*scrollbar-none[^"]*max-\[980px\]:w-auto[^"]*max-\[980px\]:mr-\[calc\(var\(--dashboard-edge-bleed\)-var\(--dashboard-scrollbar-inset\)\)\]/)
  assert.doesNotMatch(tabHistoryPanelSource, /history-entry-list pointer-events-none relative z-/)
  assert.match(tabHistoryPanelSource, /history-entry-list-content pointer-events-auto flex self-start w-65[^"]*pb-10[^"]*max-\[980px\]:pr-0[^"]*max-\[980px\]:pb-3/)
  assert.match(tabHistoryPanelSource, /history-entry-scrollbar pointer-events-none absolute top-0 right-0 bottom-0 z-20 w-\(--dashboard-scrollbar-size\) select-none max-\[980px\]:right-\[calc\(0px-var\(--dashboard-scrollbar-inset\)\)\]/)
...
test('TabHistoryPanel borrows current PageChip surface styling for the current entry', () => {
  ...
  assert.doesNotMatch(currentEntry, /hover:border-\(--accent-amber\)/)
  assert.doesNotMatch(currentEntry, /\bhover:bg-\(--history-entry-interaction-bg\)\b/)
  assert.doesNotMatch(currentEntry, /hover:after:opacity-100/)
