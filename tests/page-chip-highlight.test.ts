...
test('PageChip renders a title suppression marker when common title text is suppressed', () => {
  ...
  const chipTextMatch = html.match(/<span class="[^"]*\bchip-text\b[^"]*"[^>]*>/)
  assert.ok(chipTextMatch, 'chip text should render')
  assert.match(chipTextMatch[0], /data-slot="tooltip-trigger"/)
  ...
  assert.match(pageChipSource, /mode === 'tooltip'[\s\S]*chip-title-suppression-marker inline rounded-lg border-0[\s\S]*text-\[12px\][\s\S]*leading-\[inherit\][\s\S]*align-baseline/)
  assert.match(pageChipSource, /mode === 'tooltip'[\s\S]*\[box-decoration-break:clone\]/)
