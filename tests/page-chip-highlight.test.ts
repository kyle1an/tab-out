...
test('PageChip renders same-title URL variants below one visible title', () => {
  ...
  assert.doesNotMatch(titleVariantButtonMatch[1], /\bbg-neutral-500\/\[0\.045\]/)
  ...
  assert.match(pageChipSource, /variantCurrent && 'bg-neutral-100 text-tab-ink shadow-\[inset_2px_0_0_0_var\(--accent-amber\)\]'/)
  assert.match(pageChipSource, /variantActive && 'bg-neutral-600\/\[0\.075\] text-tab-ink'/)
  assert.doesNotMatch(pageChipSource, /variantCurrent && 'bg-neutral-100 shadow-\[inset_0_0_0_1px_rgba\(82,82,82,0\.42\)\]'/)
  assert.doesNotMatch(pageChipSource, /variantActive && 'bg-neutral-600\/\[0\.075\] text-tab-ink shadow-\[inset_0_0_0_1px_rgba\(115,115,115,0\.2\)\]'/)
...
test('TabHistoryPanel uses PageChip-style fade truncation and in-place title expansion', () => {
  ...
  assert.match(pageChipClampSource, /captureVisibleLineHtml\(textEl, visibleLineCount\)/)
  assert.match(pageChipClampSource, /clampedTitleLineNodes\(chipTextClamp\.lineHtml, 'chip-text'\)/)
