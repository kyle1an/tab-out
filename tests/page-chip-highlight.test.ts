...
test('PageChip hover fade appears and clears without its own transition lag', () => {
  ...
  assert.ok(chipMatch, 'page chip should render')
  assert.match(chipMatch[1], /\bhover:bg-\[var\(--chip-interaction-bg\)\]/)
...
test('PageChip keeps clickable hover background on expandable chips before expansion opens', () => {
  ...
  assert.match(chipMatch[1], /\bhover:bg-\[var\(--chip-interaction-bg\)\]/)
  assert.match(chipMatch[1], /page-chip-tooltip-open\]:bg-\[var\(--chip-interaction-bg\)\]/)
  assert.match(html, /--chip-interaction-bg:color-mix\(in srgb, var\(--card-bg\) 90%, var\(--color-neutral-600\) 10%\)/)
  assert.match(chipMatch[1], /:has\(\.chip-actions\):hover::after\]:opacity-100/)
})
...
test('PageChip renders same-title URL variants below one visible title', () => {
  ...
  assert.match(chipMatch[1], /hover:bg-\[var\(--chip-interaction-bg\)\]/)
  ...
  assert.match(titleVariantActionMatch[1], /group-hover\/title-variant-actions:pointer-events-auto/)
  assert.match(titleVariantActionMatch[1], /group-hover\/title-variant-actions:opacity-100/)
  ...
  assert.match(titleVariantButtonMatch[1], /\bbg-neutral-500\/\[0\.045\]/)
...
test('PageChip expands same-title URL variant groups in place', () => {
  ...
  assert.match(pageChipSource, /chipExpanded && cn\('page-chip-expanded absolute z-30/)
  assert.match(pageChipSource, /shadow-\[0_3px_10px_rgba\(10,10,10,0\.055\)\]/)
  assert.match(pageChipSource, /!transition-none/)
  assert.match(pageChipSource, /\[width:var\(--page-chip-expanded-width\)\]/)
  ...
  assert.match(pageChipSource, /chip-title-variant-list inline-flex max-w-full flex-col items-start gap-0\.5 pr-\[5px\] pb-1 align-top/)
  assert.match(pageChipSource, /chip-title-variant inline-flex max-w-full min-w-0 items-center gap-1 rounded-lg bg-neutral-500\/\[0\.045\]/)
...
test('PageChip gives same-title URL variant groups a folded-style expansion trigger', () => {
  ...
  assert.match(titleExpansionHitAreaMatch[0], /-my-\[5px\]/)
  assert.match(titleExpansionHitAreaMatch[0], /py-\[5px\]/)
...
test('PageChip routes saved-page mutation actions through Base UI context menus', () => {
  ...
  assert.match(pageChipSource, /\[\&\.page-chip-context-menu-open\]:bg-\[var\(--chip-interaction-bg\)\]/)
  assert.match(pageChipSource, /\[\&\.page-chip-tooltip-open\]:bg-\[var\(--chip-interaction-bg\)\]/)
...
test('TabHistoryPanel uses PageChip-style fade truncation and in-place title expansion', () => {
  ...
  assert.match(tabHistoryPanelSource, /w-\[var\(--history-entry-expanded-title-width\)\]/)
  ...
  assert.match(tabHistoryPanelSource, /function getHistoryEntryExpansionGeometry\(entryEl: HTMLElement \| null, titleEl: HTMLElement \| null, layerEl: HTMLElement \| null = null\)/)
  assert.match(tabHistoryPanelSource, /function getHistoryEntryLayerRoot\(panelEl: HTMLElement \| null\)/)
  assert.match(tabHistoryPanelSource, /const expandedEntryElement = titleExpanded \? historyEntrySurface\(true\) : null/)
  assert.match(tabHistoryPanelSource, /layerElement\?: HTMLElement \| null/)
  assert.match(tabHistoryPanelSource, /const \[historyPanelElement, setHistoryPanelElement\] = useState<HTMLElement \| null>\(null\)/)
  assert.match(tabHistoryPanelSource, /const \[historyLayerElement, setHistoryLayerElement\] = useState<HTMLElement \| null>\(null\)/)
  assert.match(tabHistoryPanelSource, /setHistoryLayerElement\(\(current\) => current === nextLayerElement \? current : nextLayerElement\)/)
  assert.match(tabHistoryPanelSource, /ref=\{setHistoryPanelElement\}/)
  assert.match(tabHistoryPanelSource, /createPortal\(expandedEntryElement, layerElement\)/)
  ...
  assert.doesNotMatch(tabHistoryPanelSource, /className="history-entry-slot relative min-w-0 flex-auto"[\s\S]*\{historyEntrySurface\(false\)\}[\s\S]*\{expandedEntryElement\}/)
  ...
  assert.match(tabHistoryPanelSource, /history-entry-expanded pointer-events-none absolute z-30/)
...
test('TabHistoryPanel borrows current PageChip surface styling for the current entry', () => {
  ...
  assert.match(defaultEntry, /group-hover\/history-row:bg-\[var\(--history-entry-interaction-bg\)\]/)
  ...
  assert.doesNotMatch(currentEntry, /\bgroup-hover\/history-row:bg-\[var\(--history-entry-interaction-bg\)\]\b/)
...
test('TabHistoryPanel borrows other-window PageChip surface styling for active non-current entries', () => {
  ...
  assert.match(activeOtherEntry, /\bbg-\[var\(--history-entry-rest-bg\)\]/)
  assert.match(activeOtherEntry, /shadow-\[0_1px_2px_rgba\(10,10,10,0\.04\)\]/)
  assert.match(activeOtherEntry, /group-hover\/history-row:bg-\[var\(--history-entry-interaction-bg\)\]/)
...
test('PageChip renders a title suppression marker when common title text is suppressed', () => {
  ...
  assert.match(chipTextExpansionHitAreaMatch[0], /-my-\[5px\]/)
  assert.match(chipTextExpansionHitAreaMatch[0], /py-\[5px\]/)
