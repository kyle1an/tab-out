...
test('chipActivationMode returns focus when there is no event', () => {
  assert.equal(chipActivationMode(undefined), 'focus')
  assert.equal(chipActivationMode(null), 'focus')
})

test('chipActivationMode treats a plain click as focus', () => {
  assert.equal(chipActivationMode({}), 'focus')
})

test('chipActivationMode: Cmd-click brings the tab into the current window (background)', () => {
  assert.equal(chipActivationMode({ metaKey: true }), 'bring-background')
})

test('chipActivationMode: Shift-click uses the new-window mode', () => {
  assert.equal(chipActivationMode({ shiftKey: true }), 'open-window')
})

test('chipActivationMode: Ctrl is not the primary modifier', () => {
  assert.equal(chipActivationMode({ ctrlKey: true }), 'focus')
})

test('chipActivationMode: Cmd plus Shift brings the tab in and switches (foreground)', () => {
  assert.equal(chipActivationMode({ metaKey: true, shiftKey: true }), 'bring-foreground')
})

test('chipActivationMode: holding both Cmd and Ctrl is ambiguous and stays focus', () => {
  assert.equal(chipActivationMode({ metaKey: true, ctrlKey: true }), 'focus')
  assert.equal(chipActivationMode({ metaKey: true, ctrlKey: true, shiftKey: true }), 'focus')
})

test('shouldSuppressSelectionForGesture: a plain click keeps selection (so drag-select still works)', () => {
  assert.equal(shouldSuppressSelectionForGesture(undefined), false)
  assert.equal(shouldSuppressSelectionForGesture(null), false)
  assert.equal(shouldSuppressSelectionForGesture({}), false)
})

test('shouldSuppressSelectionForGesture: special gestures suppress native selection', () => {
  assert.equal(shouldSuppressSelectionForGesture({ metaKey: true }), true)
  assert.equal(shouldSuppressSelectionForGesture({ metaKey: true, shiftKey: true }), true)
  assert.equal(shouldSuppressSelectionForGesture({ shiftKey: true }), true)
})

test('shouldSuppressSelectionForGesture: Ctrl gestures keep native selection', () => {
  assert.equal(shouldSuppressSelectionForGesture({ ctrlKey: true }), false)
  assert.equal(shouldSuppressSelectionForGesture({ ctrlKey: true, shiftKey: true }), false)
})
