...
test('app bootstrap paints filter shell before cached startup content and live refresh', () => {
  ...
  assert.match(appEntrySource, /loadDashboardLocalStateEffect/)
  ...
  assert.ok(appEntrySource.indexOf('attachApp()') < appEntrySource.indexOf("const runInitializeApp = Effect.fn('app.initialize')"))
  assert.doesNotMatch(appEntrySource, /Promise\.all\(/)
  assert.match(appEntrySource, /Effect\.forkChild\(\{ startImmediately: true \}\)/)
  assert.match(appEntrySource, /const historyRange = yield\* Fiber\.join\(historyRangeFiber\)/)
  assert.match(appEntrySource, /yield\* loadDashboardLocalStateEffect\(\)/)
  ...
  assert.match(appEntrySource, /appRuntime\.runPromise\(runInitializeApp\(\)\)/)
...
test('service worker maintains the startup snapshot on browser startup and tab events', () => {
  ...
  assert.match(serviceSource, /yield\* buildTabsDashboardStartupSnapshotEffect/)
...
test('startup state reads compose directly in the shared Effect runtimes', () => {
  ...
  assert.match(startupServiceSource, /loadDashboardLocalStateResultEffect\(\)/)
  assert.match(startupServiceSource, /fetchClosedTabsResultEffect\(\)\.pipe/)
