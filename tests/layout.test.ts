...
test('app bootstrap paints cached startup snapshot before live startup refresh', () => {
  ...
  assert.match(appEntrySource, /mountApp\(startupSnapshot, localState\)/)
  ...
  assert.match(appSource, /initialStartupSnapshot/)
  ...
  assert.match(refreshSource, /startupRefreshFlightRef/)
  assert.match(refreshSource, /startupRefreshPendingRef/)
  assert.match(refreshSource, /if \(startupRefreshFlightRef\.current\) return startupRefreshFlightRef\.current/)
  assert.match(refreshSource, /if \(startupRefreshFlightRef\.current === startupRefreshFlight\) startupRefreshFlightRef\.current = null/)
...
test('header controls share one size and corner radius contract', () => {
  ...
  assert.match(headerBarSource, /<SelectTrigger\s+className="[^"]*h-\(--header-control-height\)[^"]*rounded-\(--header-control-radius\)[^"]*bg-tab-card[^"]*text-\(length:--header-control-font-size\)[^"]*leading-\(--header-control-line-height\)/)
  assert.doesNotMatch(headerBarSource, /<SelectTrigger\s+className="[^"]*bg-\[rgba\(115,115,115,0\.06\)\]/)
  assert.match(headerBarSource, /<SelectContent[\s\S]*align="start"[\s\S]*className="[^"]*rounded-\(--header-control-radius\)/)
  assert.doesNotMatch(headerBarSource, /alignItemWithTrigger=\{false\}/)
  ...
  assert.doesNotMatch(headerBarSource, /aria-selected:bg-accent|aria-selected:text-accent-foreground/)
