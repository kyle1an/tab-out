...
test('extension HTML loads the Vite-built React entry', () => {
  ...
  assert.match(pageChipSource, /const roomToLeft = Math\.max\(0, rect\.right - PAGE_CHIP_EXPANDED_VIEWPORT_MARGIN_PX\)/)
  assert.match(pageChipSource, /const startMaxWidth = Math\.max\(rect\.width, roomToRight\)/)
  assert.match(pageChipSource, /const endMaxWidth = Math\.max\(rect\.width, roomToLeft\)/)
  assert.match(pageChipSource, /const shouldAnchorEnd = startCannotFit && endMaxWidth > startMaxWidth \+ PAGE_CHIP_EXPANDED_LINE_TOLERANCE_PX/)
  assert.match(pageChipSource, /x: shouldAnchorEnd \? 'end' : 'start'/)
  ...
  assert.match(pageChipSource, /PAGE_CHIP_GROUP_HOVER_BORDER = 'color-mix\(in srgb, var\(--color-neutral-600\) 22%, transparent\)'/)
  assert.match(pageChipSource, /PAGE_CHIP_GROUP_INTERACTION_CLASSES = `\$\{PAGE_CHIP_SURFACE_INTERACTION_CLASSES\} hover:outline hover:outline-1 hover:-outline-offset-1 hover:outline-\(--chip-group-hover-border\)/)
  assert.match(pageChipSource, /--chip-group-hover-border': PAGE_CHIP_GROUP_HOVER_BORDER/)
  ...
  assert.match(pageChipSource, /const chipInteractionBg = isCurrentTabOutFrame/)
  assert.match(pageChipSource, /: isCurrentActiveFrame/)
  ...
  assert.match(headerBarSource, /<SelectTrigger[\s\S]*className="[^"]*bg-tab-card/)
  ...
  assert.match(appComponentSource, /missions-divider mb-4 grid grid-cols-\[minmax\(0,1fr\)_auto_minmax\(0,1fr\)\] items-center gap-3/)
  assert.match(appComponentSource, /missions-divider-line flex min-w-0 items-center/)
  assert.match(appComponentSource, /action && 'gap-2'/)
  ...
  assert.deepEqual(uiFiles, ['button-variants.ts', 'button.tsx', 'context-menu-registry.ts', 'context-menu.tsx', 'input.tsx', 'menu-styles.ts', 'menu.tsx', 'select.tsx', 'tabs-variants.ts', 'tabs.tsx', 'tooltip.tsx'])
