...
test('extension HTML loads the Vite-built React entry', () => {
  ...
  assert.match(pageChipSource, /PAGE_CHIP_SURFACE_INTERACTION_CLASSES = 'hover:bg-\[var\(--chip-interaction-bg\)\]/)
  ...
  assert.match(pageChipSource, /max-w-\[var\(--page-chip-expanded-max-width\)\]/)
  ...
  assert.match(tabHistoryPanelSource, /group-hover\/history-row:bg-\[var\(--history-entry-interaction-bg\)\]/)
  ...
  assert.match(baseStylesheet, /\.scroll-region\s*\{[\s\S]*overflow-y: auto;[\s\S]*overflow-x: hidden;[\s\S]*overscroll-behavior-y: contain;/)
