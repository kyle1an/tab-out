...
test('extension HTML loads the Vite-built React entry', () => {
  ...
  assert.match(selectControlSource, /data-slot="select-trigger"/)
  assert.match(selectControlSource, /data-size="default"/)
  assert.match(selectControlSource, /border-input bg-transparent py-2 pr-2 pl-2\.5 text-sm whitespace-nowrap transition-colors/)
  assert.match(selectControlSource, /data-placeholder:text-muted-foreground/)
  assert.match(selectControlSource, /data-slot="select-content"/)
  assert.match(selectControlSource, /max-h-\(--available-height\) w-\(--anchor-width\) min-w-36/)
  assert.match(selectControlSource, /Select\.ScrollUpArrow/)
  assert.match(selectControlSource, /Select\.ScrollDownArrow/)
  assert.match(selectControlSource, /Select\.ItemIndicator/)
  assert.match(selectControlSource, /Select\.ItemText className="flex flex-1 shrink-0 gap-2 whitespace-nowrap"/)
  assert.match(selectControlSource, /pointer-events-none absolute right-2 flex size-4 items-center justify-center/)
  assert.match(selectControlSource, /ChevronDownIcon/)
  assert.match(selectControlSource, /ChevronUpIcon/)
  assert.match(selectControlSource, /data-slot="select-scroll-up-button"/)
  assert.match(selectControlSource, /data-slot="select-scroll-down-button"/)
  assert.match(selectControlSource, /align="start"/)
  assert.match(selectControlSource, /alignItemWithTrigger=\{false\}/)
  assert.doesNotMatch(selectControlSource, /data-align-trigger/)
