...
test('extension HTML loads the Vite-built React entry', async () => {
  ...
  assert.match(pageChipSource, /function onTitleVariantMouseEnter\(row: SameTitlePageChipRowView\) \{[\s\S]*setDefaultVariantSurfaceHover\(false\)[\s\S]*previewTitleVariant\(row\.id\)/)
  ...
  assert.match(tabHistoryPanelSource, /hover:bg-\(--history-entry-interaction-bg\)/)
  assert.doesNotMatch(tabHistoryPanelSource, /border border-\(--warm-gray\)/)
  assert.doesNotMatch(tabHistoryPanelSource, /hover:border-\(--accent-amber\)/)
