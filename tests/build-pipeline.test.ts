...
test('extension HTML loads the Vite-built React entry', () => {
  ...
  assert.deepEqual(uiFiles, ['context-menu-registry.ts', 'context-menu.tsx', 'input.tsx', 'menu-styles.ts', 'menu.tsx', 'select.tsx', 'tabs-variants.ts', 'tabs.tsx', 'tooltip.tsx'])
...
test('built extension bundle is packaged locally', () => {
  ...
  assert.equal(assetJsFiles.length, 14)
  ...
  assert.ok(assetJsFiles.some((name) => /^history-range-[A-Za-z0-9_-]+\.js$/.test(name)))
