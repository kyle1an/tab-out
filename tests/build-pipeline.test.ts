...
test('extension HTML loads the Vite-built React entry', () => {
  ...
  assert.match(pageChipSource, /function getExpandedPageChipLineHtml\(textEl: HTMLElement \| null, serializeFragment: ChipLineFragmentSerializer = expandedChipFragmentHtml\)/)
  assert.match(pageChipSource, /function getClampedPageChipLineHtml\(textEl: HTMLElement \| null\)/)
  ...
  assert.match(pageChipSource, /const capturedLineHtml = getExpandedPageChipLineHtml\(textEl\)/)
  assert.match(pageChipSource, /function expansionCapturedLinesRewrapOnExpand\(lineHtml: readonly string\[\]\)/)
  assert.match(pageChipSource, /'\.chip-title-suppression-marker, \.chip-strip-indicator\[aria-label\]'/)
  assert.match(pageChipSource, /const lineHtml = rewrapOnExpand \? \[\] : capturedLineHtml/)
  ...
  assert.match(appComponentSource, /lazy\(\(\) => import\('\.\/HistoryRangeSelect'\)/)
...
test('built extension bundle is packaged locally', () => {
  ...
  assert.equal(assetJsFiles.length, 12)
  ...
  assert.ok(assetJsFiles.some((name) => /^composite-[A-Za-z0-9_-]+\.js$/.test(name)))
