...
async function measureWorkingSetToggleCardMotion(session: CdpSession) {
  await session.send('Emulation.setDeviceMetricsOverride', {
    width: 1000,
    height: 900,
    deviceScaleFactor: 1,
  ...
  return evaluateWithNavigationRetry(session, {
    ...
    expression: `new Promise((resolve) => {
      function buttonText() {
        return (document.querySelector('.working-set-toggle')?.textContent || '').trim()
      }

      function clickToggle() {
        const button = document.querySelector('.working-set-toggle')
      ...
      function parseTranslateY(transform) {
        const translateMatch = /translate\\([^,]+,\\s*(-?[0-9.]+)px\\)/.exec(transform)
        if (translateMatch) return Number(translateMatch[1])
        const matrixMatch = /matrix\\([^,]+,[^,]+,[^,]+,[^,]+,[^,]+,\\s*(-?[0-9.]+)\\)/.exec(transform)
        return matrixMatch ? Number(matrixMatch[1]) : 0
      }

      function collectMotion() {
        const cards = Array.from(document.querySelectorAll('.domain-block.layout-moving'))
        const translateY = cards.map((card) => parseTranslateY(card.style.transform || getComputedStyle(card).transform || ''))
        const workingSetItems = Array.from(document.querySelectorAll('.working-set-layout-moving'))
        const workingSetTranslateY = workingSetItems.map((item) => parseTranslateY(item.style.transform || getComputedStyle(item).transform || ''))
        const ghosts = Array.from(document.querySelectorAll('.working-set-exit-ghost'))
        return {
          movingCount: cards.length,
          ghostCount: ghosts.length,
          bleed: !!document.querySelector('.scroll-region.card-motion-bleed'),
          workingSetMovingCount: workingSetItems.length,
          workingSetMinY: workingSetTranslateY.length ? Math.min(...workingSetTranslateY) : 0,
          workingSetMaxY: workingSetTranslateY.length ? Math.max(...workingSetTranslateY) : 0,
          minY: translateY.length ? Math.min(...translateY) : 0,
          maxY: translateY.length ? Math.max(...translateY) : 0,
          firstTransform: cards[0]?.style.transform || ''
        }
      }
      ...
      function waitForReady() {
        return new Promise((resolveReady) => {
          const start = Date.now()
          const wait = () => {
            if (document.querySelector('.working-set-toggle') && document.querySelector('.missions.is-packed') && document.querySelectorAll('.domain-block').length >= 12) {
              resolveReady(true)
            } else if (Date.now() - start > 5000) {
              resolveReady(false)
            } else {
      ...
      function waitForNoDomainCardMotion() {
        return new Promise((resolveStill) => {
          const start = Date.now()
          const wait = () => {
            if (document.querySelectorAll('.domain-block.layout-moving').length === 0 || Date.now() - start > 1200) {
              resolveStill()
            } else {
              setTimeout(wait, 50)
            }
      ...
      async function run() {
        ...
        if (!ready) {
          resolve({ ready: false, initialText: buttonText() })
          return
        }

        const initialText = buttonText()
        const showMoreClickText = clickToggle()
        await Promise.resolve()
        const showMore = collectMotion()
        await new Promise((resolveFrame) => requestAnimationFrame(resolveFrame))
        await new Promise((resolveDelay) => setTimeout(resolveDelay, 380))
        await waitForNoDomainCardMotion()

        const expandedText = buttonText()
        const showLessClickText = clickToggle()
        await Promise.resolve()
        const showLess = collectMotion()
        await new Promise((resolveFrame) => requestAnimationFrame(resolveFrame))
        await new Promise((resolveDelay) => setTimeout(resolveDelay, 380))
        await waitForNoDomainCardMotion()
...
async function readWorkingSetToggleTarget(session: CdpSession) {
  return evaluateWithNavigationRetry(session, {
    returnByValue: true,
    expression: `(() => {
      const button = document.querySelector('.working-set-toggle')
...
async function clickPointerAt(session: CdpSession, x: number, y: number) {
  await session.send('Input.dispatchMouseEvent', {
    type: 'mouseMoved',
    x,
    y
...
async function sampleWorkingSetPointerToggle(session: CdpSession) {
  const target = await readWorkingSetToggleTarget(session)
  if (!target) return { ready: false }

  await evaluateWithNavigationRetry(session, {
    expression: `(() => {
      const button = document.querySelector('.working-set-toggle')
      const neutralStyle = button ? getComputedStyle(button) : null
      const neutral = {
        background: neutralStyle?.backgroundColor || '',
        border: neutralStyle?.borderColor || '',
        color: neutralStyle?.color || ''
      }
      window.__tabOutWorkingSetToggleSamples = []
      const start = performance.now()
      function sample() {
        const current = document.querySelector('.working-set-toggle')
        const style = current ? getComputedStyle(current) : null
        const classList = current?.classList
        const background = style?.backgroundColor || ''
        const border = style?.borderColor || ''
        const color = style?.color || ''
        window.__tabOutWorkingSetToggleSamples.push({
          t: Math.round(performance.now() - start),
          moving: !!classList?.contains('working-set-layout-moving'),
          settling: !!classList?.contains('working-set-layout-settling'),
          highlighted: background !== neutral.background || border !== neutral.border || color !== neutral.color
        })
        if (performance.now() - start < 430) requestAnimationFrame(sample)
      }
      requestAnimationFrame(sample)
    })()`
  })

  await clickPointerAt(session, target.x, target.y)
  await wait(520)

  return evaluateWithNavigationRetry(session, {
    returnByValue: true,
    expression: `(() => {
      const samples = window.__tabOutWorkingSetToggleSamples || []
      const cleanupWindow = samples.filter((sample) => sample.t >= 220 && sample.t <= 430)
      const unlockedHighlights = cleanupWindow.filter((sample) => sample.highlighted && !sample.moving && !sample.settling)
      return {
        ready: true,
        clickText: ${JSON.stringify(target.text)},
        settlingCount: samples.filter((sample) => sample.settling).length,
        unlockedHighlightCount: unlockedHighlights.length,
        unlockedHighlights
      }
    })()`
  }).then((result: any) => result.result.value)
...
async function measureWorkingSetRealPointerToggleCleanup(session: CdpSession) {
  await session.send('Emulation.setDeviceMetricsOverride', {
    width: 1000,
    height: 900,
    deviceScaleFactor: 1,
  ...
  return {
    showMore: await sampleWorkingSetPointerToggle(session),
    showLess: await sampleWorkingSetPointerToggle(session)
  }
}
...
test('dashboard cards repack when the viewport resizes', async (t) => {
  ...
  const workingSetMotion = await measureWorkingSetToggleCardMotion(session)
  assert.equal(workingSetMotion.ready, true, `working set toggle should be available for card-motion smoke test: ${JSON.stringify(workingSetMotion)}`)
  assert.match(workingSetMotion.initialText, /Show more/, `working set should start collapsed: ${JSON.stringify(workingSetMotion)}`)
  assert.match(workingSetMotion.expandedText, /Show less/, `working set should expand before the collapse comparison: ${JSON.stringify(workingSetMotion)}`)
  assert.match(workingSetMotion.finalText, /Show more/, `working set should return to collapsed state after the comparison: ${JSON.stringify(workingSetMotion)}`)
  assert.ok(workingSetMotion.showMore.movingCount > 0, `Show more should animate domain cards: ${JSON.stringify(workingSetMotion)}`)
  assert.equal(workingSetMotion.showMore.ghostCount, 0, `Show more should not need collapse exit ghosts: ${JSON.stringify(workingSetMotion)}`)
  assert.equal(workingSetMotion.showMore.bleed, false, `Show more should not toggle source-switch bleed layout: ${JSON.stringify(workingSetMotion)}`)
  assert.ok(workingSetMotion.showMore.workingSetMovingCount > 0, `Show more should animate the working set toggle: ${JSON.stringify(workingSetMotion)}`)
  assert.ok(workingSetMotion.showMore.workingSetMinY < -1, `Show more working set movement should start above the final position: ${JSON.stringify(workingSetMotion)}`)
  assert.ok(workingSetMotion.showMore.minY < -1, `Show more should start domain cards above their final position: ${JSON.stringify(workingSetMotion)}`)
  assert.ok(workingSetMotion.showLess.movingCount > 0, `Show less should animate domain cards: ${JSON.stringify(workingSetMotion)}`)
  assert.ok(workingSetMotion.showLess.ghostCount > 0, `Show less should keep outgoing working set rows visible while cards animate up: ${JSON.stringify(workingSetMotion)}`)
  assert.equal(workingSetMotion.showLess.bleed, false, `Show less should not toggle source-switch bleed layout: ${JSON.stringify(workingSetMotion)}`)
  assert.ok(workingSetMotion.showLess.workingSetMovingCount > 0, `Show less should animate the working set toggle: ${JSON.stringify(workingSetMotion)}`)
  assert.ok(workingSetMotion.showLess.workingSetMaxY > 1, `Show less working set movement should start below the final position: ${JSON.stringify(workingSetMotion)}`)
  assert.ok(workingSetMotion.showLess.maxY > 1, `Show less should start domain cards below their final position: ${JSON.stringify(workingSetMotion)}`)

  const workingSetPointerCleanup = await measureWorkingSetRealPointerToggleCleanup(session)
  assert.equal(workingSetPointerCleanup.showMore.ready, true, `Show more pointer cleanup should be measurable: ${JSON.stringify(workingSetPointerCleanup)}`)
  assert.match(workingSetPointerCleanup.showMore.clickText, /Show more/, `real pointer cleanup should start from Show more: ${JSON.stringify(workingSetPointerCleanup)}`)
  assert.ok(workingSetPointerCleanup.showMore.settlingCount > 0, `Show more should keep the toggle neutral during cleanup: ${JSON.stringify(workingSetPointerCleanup)}`)
  assert.equal(workingSetPointerCleanup.showMore.unlockedHighlightCount, 0, `Show more should not briefly expose hover styling after motion cleanup: ${JSON.stringify(workingSetPointerCleanup)}`)
  assert.equal(workingSetPointerCleanup.showLess.ready, true, `Show less pointer cleanup should be measurable: ${JSON.stringify(workingSetPointerCleanup)}`)
  assert.match(workingSetPointerCleanup.showLess.clickText, /Show less/, `real pointer cleanup should continue with Show less: ${JSON.stringify(workingSetPointerCleanup)}`)
  assert.ok(workingSetPointerCleanup.showLess.settlingCount > 0, `Show less should keep the toggle neutral during cleanup: ${JSON.stringify(workingSetPointerCleanup)}`)
  assert.equal(workingSetPointerCleanup.showLess.unlockedHighlightCount, 0, `Show less should not briefly expose hover styling after motion cleanup: ${JSON.stringify(workingSetPointerCleanup)}`)
