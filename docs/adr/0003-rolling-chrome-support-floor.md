...
On 2026-09-06, the version listing included Chrome 153 while the fully rolled-out Apple silicon Stable release remained Chrome 152. A version's presence in the Stable listing alone is insufficient evidence for raising the install boundary.

## Decision

- Support the latest Chrome Stable major that has reached 100% rollout in the Apple silicon feed (`mac_arm64`). Intel Mac, Windows, and Linux feeds are outside the support policy.
- Compute the floor from Chrome's official `mac_arm64` VersionHistory releases endpoint, filtered by `fraction=1` and ordered by descending version. Use rollout history so a subsequent partial rollout does not erase the last completed rollout; early Stable entries alone must never raise the floor.
