...
- `Apps` renders standalone app/PWA windows as ordinary titled chips with raw window titles and ringed favicons, never active-tab highlighting or a low-score tier. `New tabs` (`__tab-out__`) and `Apps` (`__standalone-apps__`) are utility cards: unpinned by default but explicitly pinnable. Domain Card pins are local dashboard-ordering state, separate from Chrome's native `tab.pinned`.
