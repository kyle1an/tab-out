...
- On an initial Tabs-source load, reserve the Activation History column and render the filter/header shell before row/card content. Dashboard data, Activation History, Working Set, and recently closed rows commit through one startup snapshot so the shell does not shift and no history-only intermediate state flashes. With `focusFilter=1`, the pre-app filter input keeps that geometry and hands early typing to the real input.
- First dashboard-content paint uses the newest structurally valid cached startup snapshot across the in-session `chrome.storage.session` copy and durable `chrome.storage.local` mirror. The durable mirror has a multi-day cap, but display freshness TTL must not discard a valid warm snapshot; concurrent page/worker writes and partial mirror failures must not make a newer snapshot lose to an older one. Even the first open after a restart paints the last session's grouped snapshot before live data arrives.
...
- Duplicate handling renders one chip per effective URL with a stacked-favicon marker and preserves the exact open-copy count in labels. New-tab utility cards may bucket preserved same-URL Tab Out copies by current, Chrome-pinned, Chrome-grouped, and ordinary status. Bulk close/dedupe preserve Chrome tab groups; Tab Out dedupe preserves pinned Tab Out tabs and never closes the current active Tab Out page.
...
- Domain-card bulk suspend has the same ungrouped, non-preserved target scope as bulk close. It redirects live, not-already-suspended tabs through the detected Suspend Target and leaves Chrome tab groups untouched. Folded Page Chips expose exact-tab Reload/Duplicate through their environment pills while their group menu stays bulk-oriented; title-suppression tokens can close or suspend their matching ungrouped, non-preserved tabs.
...
- Hovering or focusing a Page Chip, Activation History row, or environment pill updates the exact-target URL preview. Page Chip expansion stays left-anchored and clamps to the right-side viewport budget rather than growing left.
